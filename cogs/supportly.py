...
class Supportly(commands.Cog):
    """Commands directly related to Supportly functionality."""
    ...
    async def setup(self, ctx):
        """
        Sets up a server for Supportly.

        You only need to run this command
        once after configuring Supportly.
        """

        if ctx.guild != self.bot.supportly_guild:
            return await ctx.send(f"You can only setup in the Supportly guild: {self.bot.supportly_guild}.")

        if self.bot.main_category is not None:
            logger.debug("Can't re-setup server, main_category is found.")
            return await ctx.send(f"{self.bot.supportly_guild} is already set up.")

        if self.bot.supportly_guild is None:
            embed = discord.Embed(
                title="Error",
                description="Supportly functioning guild not found.",
                color=self.bot.error_color,
            )
            return await ctx.send(embed=embed)

        overwrites = {
            self.bot.supportly_guild.default_role: discord.PermissionOverwrite(read_messages=False),
            self.bot.supportly_guild.me: discord.PermissionOverwrite(read_messages=True),
        }
        ...
        for level in PermissionLevel:
            ...
            for perm in permissions:
                perm = int(perm)
                if perm == -1:
                    key = self.bot.supportly_guild.default_role
                else:
                    key = self.bot.supportly_guild.get_member(perm)
                    if key is None:
                        key = self.bot.supportly_guild.get_role(perm)
                if key is not None:
                    logger.info("Granting %s access to Supportly category.", key.name)
                    overwrites[key] = discord.PermissionOverwrite(read_messages=True)

        category = await self.bot.supportly_guild.create_category(name="Supportly", overwrites=overwrites)

        await category.edit(position=0)

        log_channel = await self.bot.supportly_guild.create_text_channel(name="bot-logs", category=category)
        ...
        embed.add_field(
            name="Thanks for using Supportly!",
            value="Your server is now set up. Users can DM the bot to contact your support team.",
        )
        ...
        await ctx.send(
            ...
            "or users the ability to use Supportly.\n\n"
    ...
    async def nsfw(self, ctx):
        """Flags a Supportly thread as NSFW (not safe for work)."""
    ...
    async def sfw(self, ctx):
        """Flags a Supportly thread as SFW (safe for work)."""
    ...
    async def adduser(self, ctx, *users_arg: Union[discord.Member, discord.Role, str]):
        """Adds a user to a supportly thread
    ...
    async def removeuser(self, ctx, *users_arg: Union[discord.Member, discord.Role, str]):
        """Removes a user from a supportly thread
    ...
    async def anonadduser(self, ctx, *users_arg: Union[discord.Member, discord.Role, str]):
        """Adds a user to a supportly thread anonymously
    ...
    async def anonremoveuser(self, ctx, *users_arg: Union[discord.Member, discord.Role, str]):
        """Removes a user from a supportly thread anonymously
    ...
    async def logs(self, ctx, *, user: User = None):
        """
        Get previous Supportly thread logs of a member.
    ...
    async def freply(self, ctx, *, msg: str = ""):
        """
        Reply to a Supportly thread with variables.
    ...
    async def fareply(self, ctx, *, msg: str = ""):
        """
        Anonymously reply to a Supportly thread with variables.
    ...
    async def fpreply(self, ctx, *, msg: str = ""):
        """
        Reply to a Supportly thread with variables and a plain message.
    ...
    async def fpareply(self, ctx, *, msg: str = ""):
        """
        Anonymously reply to a Supportly thread with variables and a plain message.
    ...
    async def preply(self, ctx, *, msg: str = ""):
        """
        Reply to a Supportly thread with a plain message.
    ...
    async def pareply(self, ctx, *, msg: str = ""):
        """
        Reply to a Supportly thread with a plain message and anonymously.
    ...
    async def contact(
        ...
        *,
        ...
        if self.bot.config["dm_disabled"] in (
            ...
            logger.info("Contacting user %s when Supportly DM is disabled.", users[0])
    ...
    async def block(
        ...
        *,
        ...
        """
        Block a user or role from using Supportly.
    ...
    async def unblock(self, ctx, *, user_or_role: Union[User, Role] = None):
        """
        Unblock a user from using Supportly.
    ...
    async def repair(self, ctx):
        ...
        if thread is not None:
            ...
            await ctx.channel.edit(reason="Fix broken Supportly thread", topic=f"User ID: {user_id}")
            return await self.bot.add_reaction(ctx.message, sent_emoji)
        ...
        if m is not None:
            ...
            if len(users) == 1:
                ...
                await ctx.channel.edit(
                    reason="Fix broken Supportly thread",
                    name=name,
    ...
    async def enable(self, ctx):
        """
        Re-enables DM functionalities of Supportly.

        Undo's the `{prefix}disable` command, all DM will be relayed after running this command.
        """
        embed = discord.Embed(
            title="Success",
            description="Supportly will now accept all DM messages.",
            color=self.bot.main_color,
        )
    ...
    async def disable(self, ctx):
        """
        Disable partial or full Supportly thread functions.

        To stop all new threads from being created, do `{prefix}disable new`.
        To stop all existing threads from DMing Supportly, do `{prefix}disable all`.
        To check if the DM function for Supportly is enabled, do `{prefix}isenable`.
        """
    ...
    async def disable_new(self, ctx):
        """
        Stop accepting new Supportly threads.

        No new threads can be created through DM.
        """
        embed = discord.Embed(
            title="Success",
            description="Supportly will not create any new threads.",
            color=self.bot.main_color,
        )
    ...
    async def disable_all(self, ctx):
        """
        Disables all DM functionalities of Supportly.

        No new threads can be created through DM nor no further DM messages will be relayed.
        """
        embed = discord.Embed(
            title="Success",
            description="Supportly will not accept any DM messages.",
            color=self.bot.main_color,
        )
    ...
    async def isenable(self, ctx):
        """
        Check if the DM functionalities of Supportly is enabled.
        """

        if self.bot.config["dm_disabled"] == DMDisabled.NEW_THREADS:
            embed = discord.Embed(
                title="New Threads Disabled",
                description="Supportly is not creating new threads.",
                color=self.bot.error_color,
            )
        elif self.bot.config["dm_disabled"] == DMDisabled.ALL_THREADS:
            embed = discord.Embed(
                title="All DM Disabled",
                description="Supportly is not accepting any DM messages for new and existing threads.",
                color=self.bot.error_color,
            )
        else:
            embed = discord.Embed(
                title="Enabled",
                description="Supportly now is accepting all DM messages.",
                color=self.bot.main_color,
            )
    ...
    async def snooze(self, ctx, *, duration: UserFriendlyTime = None):
        ...
        if behavior == "move":
            ...
            if snoozed_cat_id:
                try:
                    target_category = self.bot.supportly_guild.get_channel(int(snoozed_cat_id))
                except Exception:
                    target_category = None
            ...
            if not isinstance(target_category, discord.CategoryChannel):
                try:
                    ...
                    overwrites = {
                        self.bot.supportly_guild.default_role: discord.PermissionOverwrite(view_channel=False)
                    }
                    bot_member = self.bot.supportly_guild.me
                    ...
                    target_category = await self.bot.supportly_guild.create_category(
    ...
    async def unsnooze(self, ctx, *, user: str = None):
        ...
        if user is not None:
            ...
            await ctx.send("This is not a Supportly thread.")
            logging.warning("[UNSNOOZE] Not a Supportly thread context.")
            return
    ...
    async def process_dm_supportly(self, message: discord.Message) -> None:
        # ... existing code ...
...
async def setup(bot):
    await bot.add_cog(Supportly(bot))
