...
class SupportlyHelpCommand(commands.HelpCommand):
...
class Utility(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.bot.help_command = SupportlyHelpCommand(
            command_attrs={
    ...
    async def changelog(self, ctx, version: str.lower = ""):
        """Shows the changelog of the Supportly."""
    ...
    async def about(self, ctx):
        ...
        embed.add_field(name="Support", value="[Discord](https://discord.gg/25YRFavwpj) | [GitHub](https://github.com/justsadnyx-ux/supportly) | [GitHub Pages](https://justsadnyx-ux.github.io/supportly/)", inline=False)
    ...
    async def sponsors(self, ctx):
        ...
        sponsors_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "SPONSORS.json")
        try:
            with open(sponsors_path, "r", encoding="utf-8") as f:
                data = loads(f.read())
        except (OSError, ValueError):
            return await ctx.send("No sponsors found for this project.")
    ...
    async def set_presence(self, *, status=None, activity_type=None, activity_message=None):
        ...
        if activity_type is not None and not activity_message:
            logger.warning('No activity message found whilst activity is provided, defaults to "Supportly".')
            activity_message = "Supportly"
        ...
        if activity_type == ActivityType.custom:
            ...
            activity = discord.Activity(type=activity_type, name=activity_message, url=None)
        else:
            activity = None
            ...
            try:
                ...
                if key == "snooze_behavior":
                    ...
                    if behavior == "move":
                        ...
                        if cat_id:
                            try:
                                cat_obj = self.bot.supportly_guild.get_channel(int(str(cat_id)))
    ...
    async def permissions(self, ctx):
        """
        Set the permissions for Supportly commands.
        ...
        Acceptable permission levels are:
            ...
            - **Supporter** [2] (access to core Supportly supporting functions)
        ...
        Note: You will still have to manually give/take permission to the Supportly
        ...
        if type_ == "command":
            ...
            if level > PermissionLevel.REGULAR:
                if value == -1:
                    ...
                    key = self.bot.supportly_guild.get_member(value)
                if key is not None:
                    ...
                    try:
                        ...
                        warn = discord.Embed(
                            ...
                            color=self.bot.error_color,
                            description=(
                                "I couldn't update the Supportly category permissions. "
    ...
    async def permissions_remove(
        ...
        *,
        ...
        if type_ == "level":
            if level > PermissionLevel.REGULAR:
                if value == -1:
                    logger.info("Denying @everyone access to Supportly category.")
                    try:
                        await self.bot.main_category.set_permissions(
                            self.bot.supportly_guild.default_role, read_messages=False
                        )
                    except discord.Forbidden:
                        warn = discord.Embed(
                            title="Missing Permissions",
                    ...
                    logger.info("Denying %s access to Supportly category.", user_or_role.name)
                    try:
                    ...
                    member = self.bot.supportly_guild.get_member(value)
                    if member is not None and member != self.bot.supportly_guild.me:
                        logger.info("Denying %s access to Supportly category.", member.name)
                        try:
    ...
    async def oauth(self, ctx):
        ...
        This functionality on your logviewer site is a premium feature.
        """
        await ctx.send_help(ctx.command)
    ...
    async def oauth_whitelist(self, ctx, target: Union[discord.Role, utils.User]):
        ...
        if not hasattr(target, "mention"):
            target = self.bot.get_user(target.id) or self.bot.supportly_guild.get_role(target.id)
    ...
    async def oauth_show(self, ctx):
        ...
        for id_ in whitelisted:
            ...
            if user:
                users.append(user)
            role = self.bot.supportly_guild.get_role(id_)
            if role:
                roles.append(role)
