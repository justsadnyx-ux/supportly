...
class Plugins(commands.Cog):
    """
    Plugins expand Supportly functionality by allowing third-party addons.
    ...
    https://github.com/justsadnyx-ux/supportly/wiki/Plugins
    """
    ...
    async def populate_registry(self):
        registry_path = Path(__file__).absolute().parent.parent / "plugins" / "registry.json"
        try:
            with open(registry_path, "r", encoding="utf-8") as f:
                self.registry = json.loads(f.read())
        except (OSError, ValueError):
            logger.warning("Failed to load local registry. Loading with empty registry")
            self.registry = {}
