...
class SupportlyBot(commands.Bot):
    def __init__(self):
        ...
        if not os.path.exists(log_dir):
            os.mkdir(log_dir)
        self.log_file_path = os.path.join(log_dir, "supportly.log")
    ...
    def startup(self):
        logger.line()
        logger.info("   ____                        __  _           ")
        logger.info("  / __/  __ _   ___   ___     / / (_)  ___   __")
        logger.info(" / _/   / _` | / __| / _ \   / /  | | / _ \ / /")
        logger.info("/ /__  | (_| || (__ | (_) | / /___| ||  __// / ")
        logger.info("\____/  \__,_| \___| \___/  \____/|_| \___/_/  ")
    ...
    @property
    def supportly_guild(self) -> typing.Optional[discord.Guild]:
        """
        The guild that the bot is operating in
        (where the bot is creating threads)
        """
        supportly_guild_id = self.config["supportly_guild_id"]
        if supportly_guild_id is None:
            return self.guild
        try:
            guild = discord.utils.get(self.guilds, id=int(supportly_guild_id))
            if guild is not None:
                return guild
        except ValueError:
            pass
        self.config.remove("supportly_guild_id")
        logger.critical("Invalid SUPPORTLY_GUILD_ID set.")
        return self.guild

    @property
    def using_multiple_server_setup(self) -> bool:
        return self.supportly_guild != self.guild

    @property
    def main_category(self) -> typing.Optional[discord.CategoryChannel]:
        if self.supportly_guild is not None:
            category_id = self.config["main_category_id"]
            if category_id is not None:
                try:
                    cat = discord.utils.get(self.supportly_guild.categories, id=int(category_id))
                    if cat is not None:
                        return cat
                except ValueError:
                    pass
            ...
            cat = discord.utils.get(self.supportly_guild.categories, name="Supportly")
            if cat is not None:
                self.config["main_category_id"] = cat.id
                logger.debug(
                    'No main category set explicitly, setting category "Supportly" as the main category.'
                )
                return cat
        return None
    ...
    async def on_ready(self):
        ...
        if self.using_multiple_server_setup:
            logger.info("Receiving guild ID: %s", self.supportly_guild.id)
        logger.line()
        ...
        other_guilds = [guild for guild in self.guilds if guild not in {self.guild, self.supportly_guild}]
        if any(other_guilds):
            logger.warning(
    ...
    async def _process_user_messages(self, user_id: int) -> None:
        ...
        while True:
            try:
                ...
                await self.process_dm_supportly(message)
                queue.task_done()
    ...
    async def process_dm_supportly(self, message: discord.Message) -> None:
            ...
            if hasattr(message, "message_snapshots") and message.message_snapshots:
                ...
                if thread is None:
                    ...
                    if self.config["dm_disabled"] in (DMDisabled.NEW_THREADS, DMDisabled.ALL_THREADS):
                        ...
                        logger.info(
                            "A new thread was blocked from %s due to disabled Supportly.", message.author
                        )
                        await self.add_reaction(message, blocked_emoji)
                    ...
                    if self.config["dm_disabled"] == DMDisabled.ALL_THREADS:
                        ...
                        logger.info("A message was blocked from %s due to disabled Supportly.", message.author)
                        await self.add_reaction(message, blocked_emoji)
                        return await message.channel.send(embed=embed)
                ...
                if ref_msg:
                    ...
                    if thread is None:
                        ...
                        if self.config["dm_disabled"] == DMDisabled.ALL_THREADS:
                            ...
                            logger.info(
                                "A message was blocked from %s due to disabled Supportly.", message.author
                            )
                            await self.add_reaction(message, blocked_emoji)
                            return await message.channel.send(embed=embed)
        ...
        if thread is None:
            ...
            if self.config["dm_disabled"] in (
                ...
                logger.info(
                    "A new thread was blocked from %s due to disabled Supportly.",
                    message.author,
                )
                await self.add_reaction(message, blocked_emoji)
                return await message.channel.send(embed=embed)
            ...
            if self.config["dm_disabled"] == DMDisabled.ALL_THREADS:
                ...
                logger.info(
                    "A message was blocked from %s due to disabled Supportly.",
                    message.author,
                )
                await self.add_reaction(message, blocked_emoji)
                return await message.channel.send(embed=embed)
    ...
    async def trigger_auto_triggers(self, message, channel, *, cls=commands.Context):
        message.author = self.supportly_guild.me
        ...
        message.author = self.supportly_guild.me  # Fix message so commands execute properly
    ...
    async def handle_react_to_contact(self, payload):
        ...
        if self.config["dm_disabled"] in (
            ...
            logger.info(
                "A new thread using react to contact was blocked from %s due to disabled Supportly.",
                member,
            )
    ...
    async def on_guild_channel_delete(self, channel):
        if channel.guild != self.supportly_guild:
            return
        ...
        if not self.supportly_guild.me.guild_permissions.view_audit_log:
            logger.debug(
        ...
        try:
            audit_logs = self.supportly_guild.audit_logs(limit=10, action=discord.AuditLogAction.channel_delete)
            found_entry = False
    ...
    async def post_metadata(self):
        # Metadata collection is disabled in this fork. No data is uploaded anywhere.
    ...
    async def autoupdate(self):
        ...
        if self.version < Version(latest.version):
            ...
            if self.hosting_method == HostingMethod.HEROKU:
                ...
                embed.set_footer(text=f"Updating Supportly v{self.version} -> v{latest.version}")
                ...
                if err and not res:
                    ...
                    if self.hosting_method in (
                        ...
                        embed.set_footer(
                            text=f"Updating Supportly v{self.version} " f"-> v{latest.version} {message}"
                        )
...
def main():
    ...
    try:
        ...
        logger.warning("Unable to import cairosvg, SVG image rendering will be unavailable.")
