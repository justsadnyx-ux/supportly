...
class GitHub:
    ...
    BASE : str
        GitHub API base URL.
    REPO : str
        Supportly repo URL for GitHub API.
    HEAD : str
        Supportly HEAD URL for GitHub API.
    MERGE_URL : str
        URL for merging upstream to master.
    FORK_URL : str
        URL to fork Supportly.
    STAR_URL : str
        URL to star Supportly.
    """

    BASE = "https://api.github.com"
    REPO = BASE + "/repos/justsadnyx-ux/supportly"
    MERGE_URL = BASE + "/repos/{username}/supportly/merges"
    FORK_URL = REPO + "/forks"
    STAR_URL = BASE + "/user/starred/justsadnyx-ux/supportly"
    ...
    async def update_repository(self, sha: str = None) -> Dict[str, Any]:
        """
        Update the repository from Supportly main repo.
        ...
        repo_url = self.BASE + f"/repos/{self.username}/supportly"
        status_map = {
...
class MongoDBClient(ApiClient):
    def __init__(self, bot):
        ...
        try:
            db = AsyncIOMotorClient(mongo_uri).supportly_bot
    ...
    async def validate_database_connection(self, *, ssl_retry=True):
        try:
            ...
            if "CERTIFICATE_VERIFY_FAILED" in message and ssl_retry:
                ...
                self.db = AsyncIOMotorClient(mongo_uri, tlsAllowInvalidCertificates=True).supportly_bot
