...
class Thread:
    """Represents a discord Supportly thread"""
    ...
    async def snooze(self, moderator=None, command_used=None, snooze_for=None):
        ...
        if behavior == "move":
            ...
            snoozed_cat_id = self.bot.config.get("snoozed_category_id")
            target_category = None
            guild = self.bot.supportly_guild
            if snoozed_cat_id:
                try:
    ...
    async def setup(self, *, creator=None, category=None, initial_message=None):
        ...
        overwrites = {self.bot.supportly_guild.default_role: discord.PermissionOverwrite(read_messages=False)}
    ...
    async def find_linked_messages(
        ...
        if message1 is not None:
            ...
            if not (
                ...
                logger.warning(
                    f"Message {message_id} is not a valid supportly relay message. embeds={bool(message1.embeds)}, author_url={getattr(message1.embeds[0], 'author', None) and message1.embeds[0].author.url}, color={getattr(message1.embeds[0], 'color', None)}, author={message1.author}"
                )
                raise ValueError("Thread message not found.")
...
class ThreadManager:
    """Class that handles storing, finding and creating Supportly threads."""
    ...
    async def populate_cache(self) -> None:
        for channel in self.bot.supportly_guild.text_channels:
        ...
        *,
        ...
        if thread is not None:
            ...
            channel = discord.utils.find(
                ...
                self.bot.supportly_guild.text_channels,
            )
            ...
            class _ThreadCreationMenuSelect(discord.ui.Select):
                ...
                async def callback(self, interaction: discord.Interaction):
                    ...
                    try:
                        ...
                        if cat_id:
                            guild = self.outer_thread.bot.supportly_guild
                            if guild:
                    ...
                    if selected and selected.get("type") == "command":
                        ...
                        if alias:
                            ...
                            for al in normalize_alias(alias):
                                ...
                                try:
                                    synthetic.author = (
                                        self.outer_thread.bot.supportly_guild.me or self.outer_thread.bot.user
                                    )
                                except Exception:
