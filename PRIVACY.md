...
Hey, we are the lead developers of Supportly bot. This is a look into the data we collect, the data you collect, the data other parties collect, and what can be done about any of this data.    
...
- Supportly: This application that has been made open-source.
- Supportly Team: The maintainers of Supportly.
- Bot: Your instance of the Supportly bot.
...
The Supportly Team collect some metadata to keep us updated on the number of instances that are making use of the bot and know what features we should focus on. The following is a list of data that we collect:
- Bot ID
...
As our bot is completely open-source, the part that details this behaviour is located in `bot.py > SupportlyBot > post_metadata`.
...
Data can be deleted with a request in a DM to our [support server](https://discord.gg/25YRFavwpj)'s Supportly bot.
...
The Supportly team does not track any data by users.
...
Plugins form a large part of the Supportly experience. Although we do not have any control over the data plugins collect, including plugins within our registry, all plugins are open-sourced by design. Some plugin devs may collect data beyond our control, and it is the bot owner's responsibility to check with the various plugin developers involved.

We recommend 4 external services to be used when setting up the Supportly bot.    
We have no control over the data external parties collect and it is up to the bot owner's choice as to which external service they choose to employ when using Supportly.    
